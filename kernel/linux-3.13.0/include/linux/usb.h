...
/* daveti: goodusb support */
	struct mutex goodusb_mutex;			/* mutex to protect followings */
	u8 goodusb_ready_to_go;				/* set by the netlink handler */
	u8 goodusb_interface_mask[USB_MAXINTERFACES];	/* udpated by the netlink handler */
	u8 goodusb_limited_hid;				/* updated by the netlink handler */
	u8 goodusb_security_pic_index;			/* updated by the netlink handler */
	u8 goodusb_description_index;			/* updated by the netlink handler */
	u8 goodusb_dev_enable;				/* updated by the netlink handler */
	int **goodusb_rawdescriptors_lens;		/* updated by usb_get_configuration */
	wait_queue_head_t goodusb_wait;			/* GoodUSB wait event */
};
