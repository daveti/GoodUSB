...
//daveti: for HoneyUSB logging
#define HONEYUSB_PATH_MAX_LEN	1024
static const char *HoneyUSB_log_path = "~/HoneyUSB/HoneyUSB.mtp.log";
static FILE *HoneyUSB_log;

static void 
HoneyUSB_LOG(const gchar *message, ...) 
{
  if (!HoneyUSB_log)
	return;
  ...
  va_list args;
  va_start (args, message);
  g_vfprintf (HoneyUSB_log, "[%s]", asctime(localtime(&ltime)));
  g_vfprintf (HoneyUSB_log, message, args);
  va_end (args);
  g_fprintf (HoneyUSB_log, "\n");
  fflush (HoneyUSB_log);
}
...
typedef struct {
  ...
  /* daveti: for HoneyUSB */
  char filename[HONEYUSB_PATH_MAX_LEN];
...
//daveti:
  HoneyUSB_LOG("make directory: %s", filename);
...
//daveti
  HoneyUSB_LOG("pull file: %s, local_path: %s", source, local_path);
...
//daveti
  HoneyUSB_LOG("push file: %s, local_path: %s", destination, local_path);
...
//daveti
  HoneyUSB_LOG("delete: %s", filename);
...
//daveti
  HoneyUSB_LOG("set display name file: %s, display_name: %s", filename, display_name);
...
//daveti
  HoneyUSB_LOG("open for read: %s", filename);
...
//daveti: save the filename
  memset(handle->filename, 0x0, HONEYUSB_PATH_MAX_LEN);
  memcpy(handle->filename, filename, strlen(filename));
...
//daveti
  HoneyUSB_LOG("open icon for read: %s", icon_id);
...
//daveti: save the file name
    memset(handle->filename, 0x0, HONEYUSB_PATH_MAX_LEN);
    memcpy(handle->filename, icon_id, strlen(icon_id));
...
//daveti
  HoneyUSB_LOG("seek on read filen: %s, id: %u, old_offset: %lu, offset: %ld, type: %u",
		handle->filename, id, old_offset, offset, type);
...
//daveti
  HoneyUSB_LOG("read file: %s, id: %u, offset: %lu, bytes_requested: %lu",
	handle->filename, id, offset, bytes_requested);
...
//daveti
  HoneyUSB_LOG("close read file: %s", handle->filename);
}
...
//daveti
  HoneyUSB_LOG("create file: %s", filename);
...
//daveti:
  HoneyUSB_LOG("append to file: %s", filename);
...
//daveti
  HoneyUSB_LOG("replace: %s", filename);
...
//daveti
  HoneyUSB_LOG("write file: %s, id: %u, offset: %lu, buffer_size: %lu",
		handle->filename, id, offset, buffer_size);
...
//daveti
  HoneyUSB_LOG("seek on write file: %s, id: %u, old_offset: %lu, offset: %ld, type: %u",
		handle->filename, id, old_offset, offset, type);
...
//daveti:
  HoneyUSB_LOG("close write: %s", handle->filename);
}
#endif /* HAVE_LIBMTP_1_1_6 */
...
//daveti: enable HoneyUSB logging
  HoneyUSB_log = fopen(HoneyUSB_log_path, "a+");
  if (!HoneyUSB_log)
	perror("HoneyUSB: fopen failed");

//daveti: all the callbacks we may want to use
